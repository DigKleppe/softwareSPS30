...
// Some developers have needed to reduce this value to avoid screen artifacts.
// It seems related to the # of SPI devices connected and/or wire lengths. If
// you see artifacts, try a low value, like 4 * 1000 * 1000, then increase
// until artifacts disappear.
static const unsigned int DISPLAY_REFRESH_HZ = 20000000;
static const int DISPLAY_SPI_QUEUE_LEN = 10;
static const int SPI_MAX_TRANSFER_SIZE = 32768;

#define EXAMPLE_LCD_H_RES DISPLAY_HORIZONTAL_PIXELS
#define EXAMPLE_LCD_V_RES DISPLAY_VERTICAL_PIXELS

#if CONFIG_IDF_TARGET_ESP32S3
static const gpio_num_t SPI_CLOCK = GPIO_NUM_12; // pen 10 LCD
static const gpio_num_t SPI_MOSI = GPIO_NUM_11;	 // pen 13
static const gpio_num_t SPI_MISO = GPIO_NUM_13;	 // nu
static const gpio_num_t TFT_CS = GPIO_NUM_10;	 // pen 9
static const gpio_num_t TFT_RESET = GPIO_NUM_8;	 // pen 15
static const gpio_num_t TFT_DC = GPIO_NUM_9;	 // pen 11
static const gpio_num_t TFT_BACKLIGHT = GPIO_NUM_7;
...
#else
static const lcd_rgb_element_order_t TFT_COLOR_MODE =  LCD_RGB_ELEMENT_ORDER_BGR;//  COLOR_RGB_ELEMENT_ORDER_BGR;
#endif
...
void lvgl_flush_cb(lv_display_t *display, const lv_area_t *area, uint8_t *color_map) {
	...
	esp_lcd_panel_draw_bitmap(lcd_handle, offsetx1, offsety1, offsetx2 + 1, offsety2 + 1, color_map);
}

static void IRAM_ATTR lvgl_tick_cb(void *param) { lv_tick_inc(LVGL_UPDATE_PERIOD_MS); }

static void display_brightness_init(void) {
	const ledc_channel_config_t LCD_backlight_channel = {.gpio_num = TFT_BACKLIGHT,
	...
	const ledc_timer_config_t LCD_backlight_timer = {.speed_mode = BACKLIGHT_LEDC_MODE,
...
void initialize_spi() {
	...
	spi_bus_config_t bus = {.mosi_io_num = SPI_MOSI,
...
void initialize_display() {
	const esp_lcd_panel_io_spi_config_t io_config = {.cs_gpio_num = TFT_CS,
													 .dc_gpio_num = TFT_DC,
													 .spi_mode = 0,
	...
	const esp_lcd_panel_dev_config_t lcd_config = {.reset_gpio_num = TFT_RESET,
...
void initialize_lvgl() {
	...
	lv_init();
	lv_display = lv_display_create(EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES);
